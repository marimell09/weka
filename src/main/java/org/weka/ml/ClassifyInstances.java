...
public class ClassifyInstances extends Application {
	...
	public void start(Stage stage) throws Exception {
		...
		// ObjectOutputStream oos = new ObjectOutputStream(baos);
		 ...
		 System.out.println("Excrevendo num arquivo arff a predição"); 
		...
		SwingUtilities.invokeLater(() -> {
			...
			treeVisualizer.setPreferredSize(new Dimension(300, 300));
			sw.setContent(treeVisualizer);
		});

	
		StackPane spTree = new StackPane(sw);
		spTree.setPrefWidth(400);
		spTree.setPrefHeight(350);
		spTree.cacheShapeProperty();
		
		VBox vbDecisionTree = new VBox(5, lblDecisionTreeTitle, new Separator(), spTree);
		
		
		vbDecisionTree.setPrefHeight(500);
		vbDecisionTree.setPrefWidth(1000);

		GridPane gpRoot = new GridPane();
		gpRoot.add(vbDecisionTree, 0,0);
