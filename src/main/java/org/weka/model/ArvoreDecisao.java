...
public class ArvoreDecisao {
	...
	public Instances classificarDados(Instances dadosParaClassificar, Classifier cls){
		
		
		//System.out.println("value" + " -> " + "prediction");
		try {
			for (int i = 0; i < dadosParaClassificar.numInstances(); i++) {
				double clsLabel = cls.classifyInstance(dadosParaClassificar.instance(i));
				dadosParaClassificar.instance(i).setClassValue(clsLabel);
				
				// System.out.println(dadosParaClassificar.instance(i).value(0) + " -> " + dadosParaClassificar.classAttribute().value((int) clsLabel));
			}
